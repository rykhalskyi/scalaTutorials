...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted:Tweet={
    def maxLeft(elem:Tweet):Tweet = {
      left match {
        case s : Empty => elem
        case s : NonEmpty => {
          val mostLeft = s.mostRetweeted
          if (mostLeft.retweets > elem.retweets) mostLeft else elem
        }
      }
    }
      
      def maxRight(elem:Tweet):Tweet = {
      right match {
        case s : Empty => elem
        case s : NonEmpty => {
          val mostRight = s.mostRetweeted
          if (mostRight.retweets > elem.retweets) mostRight else elem
        }
      }
     }
    
      val _maxLeft = maxLeft(elem)
      val _maxRight = maxRight(elem)
      if (_maxLeft.retweets >= _maxRight.retweets) _maxLeft else _maxRight
